...
assert(db:exec[[
	CREATE TABLE IF NOT EXISTS states(
		...
		screen_jpg BLOB,
		score INTEGER(12),
		death_count INTEGER(6),
		level_beaten_count INTEGER(6),
		player_x INTEGER(6),
		world_count INTEGER(12),
...
-- copied from above, but returns maxWorldCount
-- TODO: needed?
function memory.getMaxWorldCount(defaultVal)
	if memory.getCountAllEntries() == 0 then
		return defaultVal
	else
		for row in db:nrows(string.format("SELECT MAX(world_count) AS world_count FROM states")) do
			return row.world_count
		end
	end
end
...
function memory.insertStatesBatched(states, validation, updateIfExists)
	...
	for i=1,#states,batchSize do
		...
		for j=i,math.min(i+batchSize, #states) do
			...
			data["deathcount" .. j] = state.deathCount
			data["levelbeatencount" .. j] = state.levelBeatenCount
			data["playerx" .. j] = state.playerX
			data["worldcount" .. j] = state.worldCount
...
function memory.rowToState(row)
	...
	local action = Action.new(row.action_arrow)
	...
	local state = State.new(row.id, screen_jpg, row.score, row.death_count, row.level_beaten_count, row.player_x, row.world_count, action, reward)
